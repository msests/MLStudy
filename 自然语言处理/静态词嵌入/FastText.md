...
+ **随机初始化**：每个子词向量都是随机初始化的，并且在训练阶段通过反向传播进行更新。
...
| ​**优点**                          | ​**缺点**                     |
|-----------------------------------|------------------------------|
| 可处理OOV词                       | 存储开销较大（需存储所有子词向量） |
| 对形态丰富语言效果更好            | 长词计算成本较高              |
| 通过子词共享提升小数据集泛化能力  | 对短词（n < gram长度）不友好  |

## 数学符号说明

- $d$：词向量维度
- $G_w$：词$w$的子词集合
- $\mathbf{v}_g$：子词$g$的向量
- $\mathbf{c}$：上下文词的输出向量
